"""
CGV IMAX 예매 알림 시스템 v2.0 - Vercel 배포용
Vercel Serverless Functions 환경에 최적화
"""
...
# SSE 이벤트 큐 (Vercel에서는 제한적)
sse_queues = {}

# 모니터링 상태 (Vercel에서는 상태 저장이 제한적이므로 DB 사용)
...
# ============================================================================
# SSE (Server-Sent Events) - Vercel에서는 제한적
# ============================================================================
...
# ============================================================================
# 모니터링 작업 (Vercel Cron으로 실행)
# ============================================================================

def monitoring_job():
    """백그라운드 모니터링 작업 - Vercel Cron에서 호출"""
    try:
        with app.app_context():
            # 감시 중인 영화들 조회
            ...
            if not monitored_movies:
                ...
                return {'status': 'no_movies', 'message': '감시 중인 영화 없음'}
            ...
            return {
                'status': 'success',
                'movies_checked': len(monitored_movies),
                'results': results
            }
...
def cron_monitor():
    ...
    주기적으로 영화 예매 상태 확인
    """
    # Vercel Cron의 Authorization 헤더 확인 (보안)
    ...
    logger.info("Vercel Cron: 모니터링 작업 시작")
    result = monitoring_job()
...
def start_monitoring():
    ...
    try:
        ...
        # Vercel Cron이 자동으로 실행하므로 별도 스케줄러 불필요
        logger.info(f"모니터링 활성화: {monitored_count}개 영화 (Vercel Cron 사용)")

        # SSE 이벤트
        send_sse_event('monitoring_started', {'interval': prefs.check_interval or 180})

        return jsonify({
            'success': True,
            'message': '모니터링 시작 (Vercel Cron 사용)',
            'monitored_count': monitored_count,
            'interval': prefs.check_interval or 180,
            'note': 'Vercel Cron이 설정된 간격으로 자동 실행합니다'
        })
...
def stop_monitoring():
    ...
    try:
        # 모든 영화의 모니터링 비활성화
        Movie.query.update({Movie.is_monitoring: False})
        db.session.commit()
...
# Vercel에서는 각 요청마다 새로운 인스턴스가 생성될 수 있으므로
# 앱 컨텍스트 내에서 DB 초기화
try:
    with app.app_context():
        db.create_all()
        logger.info("Vercel: 데이터베이스 테이블 확인/생성 완료")
except Exception as e:
    logger.error(f"Vercel: 데이터베이스 초기화 오류: {e}")
